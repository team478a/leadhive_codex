...
export interface FormCodexField { position: number; label: string; name: string; field_type: string; required: boolean; mapped_key: string; value: string; options: string[] }
export interface FormCodexPayload { task_reference: string; skill_name: string; submission_authorized: boolean; company_name: string; form_url: string; subject: string; body: string; reason: string; sender_values: Record<string, string>; fields: FormCodexField[]; instructions: string }
...
export interface FormProfile { id: string; company_id: string; form_url: string; form_index: number; form_status: FormStatus; sales_contact_status: 'ALLOWED' | 'PROHIBITED' | 'UNCERTAIN'; captcha_type: 'CAPTCHA_NONE' | 'CAPTCHA_RECAPTCHA' | 'CAPTCHA_HCAPTCHA' | 'CAPTCHA_TURNSTILE' | 'CAPTCHA_OTHER'; confirmation_page: boolean | null; is_primary: boolean; form_found: boolean; page_kind: string; fingerprint: string; analysis_version: string; analysis_provider: string; last_analyzed_at: string | null; analysis_duration_ms: number; delivery_supported: boolean; review_reason: string; error_message: string; created_at: string; updated_at: string; fields: FormProfileField[] }
...
export type FormAssist = FormCodexPayload
