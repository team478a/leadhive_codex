...
import type { Activity, CollectionSource, Company, CompanyPage, Project, SalesStatus } from './types'
...
export function CompaniesPage({ projects, initialProjectId }: { projects: Project[]; initialProjectId: string }) {
  ...
  const reload = useCallback(async () => {
    ...
    const result = await api<CompanyPage>(`/projects/${projectId}/company-list?${query}`)
    setCompanies(result.items); setTotal(result.total); setChecked([])
  }, [projectId, query])
