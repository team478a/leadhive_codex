...
export function DashboardPage({ onUnreadChange, onOpenInboundReply, onOpenFollowup }: {
  onUnreadChange: (count: number) => void
  onOpenInboundReply: (projectId: string, inboundEmailId: string) => void
  onOpenFollowup: (projectId: string, companyId: string) => void
}) {
  ...
  async function openFollowup(item: Notification) {
    ...
    try {
      ...
      onOpenFollowup(item.project_id, item.company_id)
    } catch (e) { setError(errorMessage(e)) }
  }
